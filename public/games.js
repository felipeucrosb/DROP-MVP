...
window.DropGames = (() => {
  ...
  function stack(ctx) {
    ...
    function move(now=performance.now()){if(dead)return;const dt=Math.min(32,now-last)/16.67;last=now;const sec=survivalHud(start),blockRamp=1+Math.max(0,n-5)*.18+Math.max(0,n-8)*.42;const speed=2.35*survivalCurve(sec)*blockRamp;x+=speed*dir*dt;if(x<0){x=0;dir=1}if(x+w>box.clientWidth){x=box.clientWidth-w;dir=-1}cur.style.left=x+"px";raf=requestAnimationFrame(move)}
    box.addEventListener("pointerdown",e=>{e.preventDefault();if(dead)return;cancelAnimationFrame(raf);if(n===0){lastX=x;n++;spawn();return}const L=Math.max(x,lastX),R=Math.min(x+w,lastX+w),overlap=R-L;if(overlap<=3){dead=true;const ms=performance.now()-start;ctx.submit(ms,`${(ms/1000).toFixed(2)} sec · ${n} blocks`);return}w=Math.max(4,overlap-(n>=6?(n-5)*2.4:0));lastX=L+(overlap-w)/2;cur.style.left=lastX+"px";cur.style.width=w+"px";n++;ctx.live(Math.round(performance.now()-start));spawn()},{passive:false});spawn();
  }
  ...
  function flappy(ctx) {
    ...
    box.addEventListener("pointerdown",e=>{e.preventDefault();if(!dead)vy=-7},{passive:false});box.addEventListener("dblclick",e=>e.preventDefault(),{passive:false});
